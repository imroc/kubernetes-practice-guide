...
* 通过 `nsenter` 进入 pod netns，使用节点上的 tcpdump 抓 pod 中的包 \(抓包方法参考[这里](/troubleshooting/trick/capture-packets-in-container.md)\)，发现是有的 DNS 请求没有收到响应，超时 5 秒后，再次发送 DNS 请求才成功收到响应。
