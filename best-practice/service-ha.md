...
k8s 的设计就是假设节点是不可靠的，节点数量多的话发生软硬件故障导致节点不可用的几率就更高，所以我们通常需要给服务部署多个副本，根据实际情况调整 `replicas` 的值，如果值为 1 就必然存在单点故障，如果大于 1 但所有副本都调度到同一个节点，那还是有单点故障，所以我们不仅要有合理的副本数量，还需要让这些不同副本调度到不同的节点，避免单点故障，这个可以利用反亲和性来实现:
